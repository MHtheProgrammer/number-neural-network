...
class MLP:
    ...
    def sig_prime(self, input: np.ndarray):
        '''
    ...
    def calculate_and_sum_gradients(self):
        ...
        sig_prime_output = self.sig_prime(self.output_nodes)
        sig_prime_hidden = self.sig_prime(self.hidden_nodes)
        ...
        dc_da_hidden[Constants.HIDDEN_LAYER_COUNT-1] = np.multiply(dc_da_output, sig_prime_output) @ np.transpose(self.output_weights)
        
        # Now for the hidden layers
        for i in range(Constants.HIDDEN_LAYER_COUNT - 2, -1, -1):
            dc_da_hidden[i] = np.multiply(dc_da_hidden[i+1], sig_prime_hidden[i+1]) @ np.transpose(self.hidden_weights[i])

        # Done calculating dc/da values
        # Now we calculate dc/dw
        
        # First the output weights
        dc_dw_output = np.outer(self.hidden_nodes[Constants.HIDDEN_LAYER_COUNT-1], np.multiply(dc_da_output, sig_prime_output))
        ...
        for i in range(Constants.HIDDEN_LAYER_COUNT-2, -1, -1):
            dc_dw_hidden[i] = np.outer(self.hidden_nodes[i], np.multiply(dc_da_hidden[i+1], sig_prime_hidden[i+1]))

        # Now the input weights
        dc_dw_input = np.outer(self.input_nodes, np.multiply(dc_da_hidden[0], sig_prime_hidden[0]))

        # Done calculating dc/dw
        # Now calculate dc/db
        dc_db_output = np.multiply(dc_da_output, sig_prime_output)
        dc_db_hidden = np.multiply(dc_da_hidden, sig_prime_hidden)
